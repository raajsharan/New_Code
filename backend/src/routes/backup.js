...
async function buildDecryptedPasswordPatch() {
  try {
    const { rows } = await pool.query('SELECT id, asset_password FROM assets WHERE asset_password IS NOT NULL AND asset_password <> \'\'');
    if (!rows.length) return '';
    const sqe = (v) => String(v ?? '').replace(/'/g, "''");
    const updates = rows
      .map(r => {
        const plain = decryptPassword(r.asset_password);
        if (!plain) return null;
        return `UPDATE assets SET asset_password = '${sqe(plain)}' WHERE id = ${r.id};`;
      })
      .filter(Boolean);
    ...
    return [
      '',
      '-- ============================================================',
      '-- Decrypted asset passwords (generated by InfraInventory Backup)',
      '-- WARNING: passwords below are stored in plaintext.',
      '-- Remove this section before restoring to a shared environment.',
      '-- ============================================================',
      ...updates,
      '',
    ].join('\n');
  ...
  if (include_assets) {
    const r = await pool.query(`
      SELECT a.id, a.vm_name, a.os_hostname, a.ip_address,
        ...
        a.hosted_ip, a.asset_username, a.asset_password, a.additional_remarks,
    ...
    results.assets = r.rows.map(row => ({
      ...row,
      asset_password: row.asset_password ? (decryptPassword(row.asset_password) || '') : '',
    }));
  }
  ...
  if (include_ext) {
    ...
    const r = await extPool.query(`
      SELECT i.id, i.vm_name, i.asset_name, i.os_hostname, i.ip_address,
        ...
        i.asset_username, i.asset_password, i.submitted_by, i.created_at, i.updated_at,
...
async function runScheduledTasks() {
  ...
  if (schedule.pg_enabled && shouldRunNow('pg', schedule.pg_frequency, schedule.pg_time, now)) {
    ...
    try {
      ...
      fs.writeFileSync(fullPath, fullDump, 'utf8');
      cleanupOldPgBackups(backupDir, schedule.pg_retain_days);
      await appendBackupLog({
        type: 'pg_dump',
        status: 'success',
        filename,
        details: `${fullPath} (${(Buffer.byteLength(fullDump, 'utf8') / 1024 / 1024).toFixed(2)} MB)`,
