...
const DEFAULT_FIELD_LAYOUT = {
  ...
  hosted_ip:                {group:'Host Details', sort:5},
...
const ASSET_INIT = {
  ...
  eol_status:'', asset_username:'', asset_password:'', custom_field_values:{},
...
// 
function renderFormGroups({ layout, form, set, dropdowns, osVersions, canWrite, showPw, setShowPw, dupState, checking, tagValidation, setTagValidation, editAsset, handleIPChange, oemOptions, fieldTypeOverrides, customFields = [], setCustom = () => {}, setShowNameConversion }) {
  ...
  // Renders a field that may be textdropdown or textradio based on override
  ...
  // Build a map of custom fields by their assigned group
  const cfGroupMap = {};
        ...
        <Field key={key} label="OS Hostname">
          ...
          <p className="text-xs text-gray-500 mt-2">
            <button type="button" onClick={()=>setShowNameConversion(true)} className="text-blue-600 hover:text-blue-700 font-semibold underline inline-flex items-center gap-1">
              <ExternalLink size={11}/> Name Convention
            </button>
          </p>
        </Field>
      );
...
const StatusBadge = ({ s }) => {
  ...
  const cls = { 'Alive':'bg-green-100 text-green-700','Powered Off':'bg-orange-100 text-orange-700','Not Alive':'bg-red-100 text-red-700' }[safe]||'bg-gray-100 text-gray-500';
...
const ASSET_COL_DEFAULTS = [
  ...
  {key:'os_type',        label:'OS',                visible:true},
  {key:'os_version',     label:'OS Version',        visible:true},
  {key:'assigned_user',  label:'Assigned User',     visible:true},
  {key:'department',     label:'Department',        visible:true},
  {key:'server_status',  label:'Status',            visible:true},
  {key:'patching_type',  label:'Patch Type',        visible:true},
  {key:'server_patch_type', label:'Ser. Patch Type',visible:true},
  {key:'patching_schedule', label:'Schedule',       visible:true},
  {key:'location',       label:'Location',          visible:true},
  {key:'serial_number',  label:'Serial Number',     visible:true},
  {key:'idrac',          label:'iDRAC',             visible:true},
  {key:'oem_status',     label:'OME',               visible:true},
  {key:'eol_status',     label:'EOL',               visible:true},
  {key:'me_installed',   label:'ME',                visible:true},
  {key:'tenable_installed', label:'Tenable',        visible:true},
  {key:'hosted_ip',      label:'Hosted IP',         visible:true},
  {key:'asset_tag',      label:'Asset Tag',         visible:true},
  ...
  {key:'asset_password', label:'Password',          visible:true},
  {key:'submitted_by',   label:'Submitted By',      visible:false},
  {key:'updated_at',     label:'Last Modified',     visible:true},
];
...
function mergeAssetColConfig(saved){
  ...
  const merged=ASSET_COL_DEFAULTS.map((d,i)=>({...d,visible:d.key==='asset_password'?true:(savedMap[d.key]!==undefined?savedMap[d.key].visible:d.visible),order:savedMap[d.key]!==undefined?savedMap[d.key].order:999+i}));
...
function AssetListTab({ onEdit, refreshKey }) {
  ...
  const resetBulkPatch = () => setBulkPatch({
    assigned_user:'', department_id:'', server_status_id:'', patching_type_id:'',
    patching_schedule_id:'', location_id:'', eol_status:'',
  });
  const pollBulkJob = async (jobId) => {
    for (let i = 0; i < 20; i++) {
      const jr = await assetsAPI.getBulkJob(jobId);
      const j = jr.data?.job;
      if (!j) break;
      setBulkJob(j);
  ...
  const handleBulkUpdate = async (dryRun = false) => {
    ...
    if (!dryRun && !confirm('Apply this bulk update to all assets matching current filters?')) return;

    const activeFilters = { ...filters };
    Object.keys(activeFilters).forEach(k => { if (!activeFilters[k]) delete activeFilters[k]; });

    if (dryRun) setBulkDryRunning(true);
    else setBulkSubmitting(true);
    try {
      const r = await assetsAPI.bulkUpdate({ filters: activeFilters, patch, dry_run: dryRun });
      const initial = r.data || {};
      if (initial.job_id) {
        setBulkJob({
          id: initial.job_id,
          status: initial.status,
          total_count: initial.matched_count,
          success_count: initial.success_count || 0,
          failed_count: initial.failed_count || 0
        });
      }

      if (dryRun) {
        toast.success(`Dry run complete: ${Number(initial.matched_count || 0)} assets would be targeted`);
      } else {
      ...
      if (dryRun) setBulkDryRunning(false);
      else setBulkSubmitting(false);
    }
  };
  const canViewPw = user?.can_view_passwords || user?.role === 'admin' || user?.role === 'superadmin';
  const totalPages=Math.ceil(total/limit);
  const COL_VM=140,COL_IP=120;
  const renderCustomVal=(a,cf)=>{const raw=a.custom_field_values?.[cf.field_key];if(isBlankLike(raw))return<span className="text-gray-400 text-xs">-</span>;if(cf.field_type==='toggle')return raw?<span className="text-green-600 text-xs">Yes</span>:<span className="text-gray-400 text-xs">No</span>;return<span className="text-xs text-gray-700">{displayText(raw)}</span>;};
        ...
        <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4" onClick={()=>!bulkSubmitting&&setShowBulkModal(false)}>
          <div className="bg-white rounded-xl shadow-xl border border-gray-200 w-full max-w-2xl p-5" onClick={e=>e.stopPropagation()}>
            <div className="flex items-start justify-between mb-4">
              <div>
                <h3 className="text-lg font-semibold text-gray-800">Bulk Update Assets</h3>
                <p className="text-xs text-gray-500 mt-1">Applies to assets matching current filters.</p>
              </div>
            ...
            <tbody className="divide-y divide-gray-100">
              ...
              :assets.map(a=>(
                <tr key={a.id} className={`hover:bg-blue-50/20 ${selected.has(a.id)?'bg-blue-50':''}`}>
                  ...
                  {colConfig.filter(c=>c.visible && c.key!=='actions').map(c=>{
                    switch(c.key){
                      case 'os_hostname':    return <td key={c.key} className="table-td font-mono text-xs">{displayText(a.os_hostname)}</td>;
                      case 'asset_type':     return <td key={c.key} className="table-td text-xs">{displayText(a.asset_type)}</td>;
                      case 'os_type':        return <td key={c.key} className="table-td text-xs">{displayText(a.os_type)}</td>;
                      case 'os_version':     return <td key={c.key} className="table-td text-xs max-w-[130px] truncate">{displayText(a.os_version)}</td>;
                      case 'assigned_user':  return <td key={c.key} className="table-td text-xs">{displayText(a.assigned_user)}</td>;
                      case 'department':     return <td key={c.key} className="table-td text-xs">{displayText(a.department)}</td>;
                      ...
                      case 'server_patch_type': return <td key={c.key} className="table-td text-xs">{displayText(a.server_patch_type)}</td>;
                      case 'patching_schedule': return <td key={c.key} className="table-td text-xs">{displayText(a.patching_schedule)}</td>;
                      case 'location':       return <td key={c.key} className="table-td text-xs">{displayText(a.location)}</td>;
                      case 'serial_number':  return <td key={c.key} className="table-td font-mono text-xs">{displayText(a.serial_number)}</td>;
                      ...
                      case 'oem_status':     return <td key={c.key} className="table-td">{a.oem_status?<span className={`px-2 py-0.5 rounded-full text-xs font-medium ${a.oem_status==='YES'?'bg-green-100 text-green-700':a.oem_status==='NO'?'bg-red-100 text-red-600':'bg-gray-100 text-gray-500'}`}>{a.oem_status}</span>:<span className="text-gray-400 text-xs">-</span>}</td>;
                      case 'eol_status':     return <td key={c.key} className="table-td text-xs"><span className={a.eol_status==='InSupport'?'text-green-700':a.eol_status==='EOL'?'text-orange-600':'text-red-600'}>{displayText(a.eol_status)}</span></td>;
                      ...
                      case 'hosted_ip':      return <td key={c.key} className="table-td">{!isBlankLike(a.hosted_ip)?<div className="flex items-center gap-1"><button onClick={()=>navigate(`/physical-assets?ip=${encodeURIComponent(displayText(a.hosted_ip,''))}`)} className="text-xs font-mono text-blue-700 hover:underline flex items-center gap-1"><Server size={11}/>{displayText(a.hosted_ip)}</button><a href={`https://${displayText(a.hosted_ip,'')}/ui/#/login`} target="_blank" rel="noopener noreferrer" className="p-0.5 text-gray-400 hover:text-blue-600"><ExternalLink size={11}/></a></div>:<span className="text-gray-400 text-xs">-</span>}</td>;
                      case 'asset_tag':      return <td key={c.key} className="table-td font-mono text-xs font-medium text-purple-700">{displayText(a.asset_tag)}</td>;
                      case 'business_purpose': return <td key={c.key} className="table-td text-xs text-gray-600 max-w-[140px] truncate" title={displayText(a.business_purpose, '')}>{displayText(a.business_purpose)}</td>;
                      case 'additional_remarks': return <td key={c.key} className="table-td text-xs text-gray-600 max-w-[160px] truncate" title={displayText(a.additional_remarks, '')}>{displayText(a.additional_remarks)}</td>;
                      case 'asset_username': return <td key={c.key} className="table-td font-mono text-xs">{displayText(a.asset_username)}</td>;
                      case 'asset_password': return <td key={c.key} className="table-td"><PasswordCell password={a.asset_password} canView={canViewPw}/></td>;
                      case 'submitted_by':   return <td key={c.key} className="table-td text-xs text-gray-500">{displayText(a.submitted_by)}</td>;
                  ...
                  <td className="table-td">
                    <div className="flex gap-1 justify-center">
                      <button onClick={()=>onEdit(a)} className="p-1.5 text-blue-600 hover:bg-blue-100 rounded transition-colors" title="Edit"><Edit2 size={13}/></button>
                      {canWrite&&<button onClick={()=>handleDelete(a)} className="p-1.5 text-red-500 hover:bg-red-100 rounded transition-colors" title="Delete"><Trash2 size={13}/></button>}
                    </div>
                  </td>
                </tr>
              ))}
