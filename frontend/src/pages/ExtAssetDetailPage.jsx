...
import { extendedInventoryAPI, auditAPI } from '../services/api';
import { useAuth } from '../context/AuthContext';
import { MEIcon, TenableIcon } from '../components/AgentIcon';
import toast from 'react-hot-toast';
import {
  ArrowLeft, Edit2, Layers, Shield, Zap, Building2,
  HardDrive, Key, Tag, ExternalLink, RefreshCw, Server
} from 'lucide-react';
...
const Section = ({ icon: Icon, title, color='text-indigo-700', children }) => (
  <div className="card">
...
const statusCls  = s => ({'Active':'bg-green-100 text-green-700','Inactive':'bg-gray-100 text-gray-500','Decommissioned':'bg-red-100 text-red-600','Maintenance':'bg-amber-100 text-amber-700'}[s]||'bg-gray-100 text-gray-500');
const srvStatCls = s => ({'Alive':'bg-green-100 text-green-700','Powered Off':'bg-orange-100 text-orange-700','Not Alive':'bg-red-100 text-red-700'}[s]||'bg-gray-100 text-gray-500');
...
export default function ExtAssetDetailPage() {
  const { id } = useParams();
  ...
  const summarizeAudit = (log) => {
    if (log.action === 'create') return 'Extended item created';
    if (log.action === 'delete') return 'Extended item deleted';
    if (log.action === 'transfer') return 'Transferred to main inventory';
  ...
  const fetchItem = useCallback(async () => {
    setLoading(true);
    try {
      const r = await extendedInventoryAPI.getById(id);
      setItem(r.data);
    } catch {
      toast.error('Record not found');
      navigate('/ext-asset-list');
  ...
  const fetchAuditLogs = useCallback(async () => {
    ...
    try {
      const r = await auditAPI.getEntityLogs('ext_item', id, { page: 1, limit: 20 });
  ...
  useEffect(() => { fetchItem(); fetchAuditLogs(); }, [fetchItem, fetchAuditLogs]);
  ...
  const customEntries = Object.entries(item.custom_field_values || {}).filter(([,v]) => v !== '' && v !== null && v !== undefined);
    ...
    <div className="max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-800">{item.vm_name || item.asset_name || `Ext. Record #${item.id}`}</h1>
            <div className="flex items-center gap-2 mt-0.5">
              <code className="text-sm font-mono text-indigo-700 bg-indigo-50 px-2 py-0.5 rounded">{item.ip_address}</code>
              {item.status && (
                <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${statusCls(item.status)}`}>{item.status}</span>
              )}
              {item.asset_tag && (
                <span className="text-xs px-2 py-0.5 rounded-full bg-purple-100 text-purple-700 font-medium font-mono">#{item.asset_tag}</span>
              )}
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        {[
          { label: 'Asset Type',  value: item.asset_type,   cls: 'bg-indigo-50 text-indigo-800' },
          { label: 'OS',          value: `${item.os_type||''}${item.os_version?' · '+item.os_version:''}` || '—', cls: 'bg-violet-50 text-violet-800' },
          { label: 'Department',  value: item.department,   cls: 'bg-teal-50 text-teal-800' },
          { label: 'Location',    value: item.location,     cls: 'bg-amber-50 text-amber-800' },
        ].map(s => (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        <Section icon={Layers} title="Basic Information">
          <InfoRow label="VM / Asset Name" value={item.vm_name || item.asset_name} mono/>
          <InfoRow label="OS Hostname"     value={item.os_hostname} mono/>
          <InfoRow label="IP Address"      value={item.ip_address} mono/>
          <InfoRow label="Asset Type"      value={item.asset_type}/>
          <InfoRow label="OS Type"         value={item.os_type}/>
          <InfoRow label="OS Version"      value={item.os_version}/>
        </Section>

        {/* Ownership */}
        <Section icon={Building2} title="Ownership" color="text-teal-700">
          <InfoRow label="Assigned User"    value={item.assigned_user}/>
          <InfoRow label="Department"       value={item.department}/>
          <InfoRow label="Business Purpose" value={item.business_purpose}/>
          <InfoRow label="Asset Tag"        value={item.asset_tag} mono/>
          <InfoRow label="Serial Number"    value={item.serial_number} mono/>
        </Section>

        {/* Status */}
        <Section icon={Shield} title="Status & Patching" color="text-green-700">
          <InfoRow label="Record Status"  value={item.status}         badge badgeCls={statusCls(item.status)}/>
          <InfoRow label="Server Status"  value={item.server_status}  badge badgeCls={srvStatCls(item.server_status)}/>
          <InfoRow label="Patching Type"  value={item.patching_type}/>
          <InfoRow label="Patch Schedule" value={item.patching_schedule}/>
          <InfoRow label="Patch Type"     value={item.server_patch_type}/>
          <InfoRow label="Location"       value={item.location}/>
          <InfoRow label="EOL Status"     value={item.eol_status} badge badgeCls={eolCls(item.eol_status)}/>
        </Section>
        ...
        <Section icon={Zap} title="Agent Status" color="text-amber-600">
          <div className="flex items-center py-2 border-b border-gray-50">
            ...
            <MEIcon installed={item.me_installed_status} size={20}/>
            <span className="ml-2 text-sm text-gray-700">{item.me_installed_status ? 'Installed' : 'Not Installed'}</span>
          </div>
          <div className="flex items-center py-2">
            <span className="text-xs font-medium text-gray-500 w-40 flex-shrink-0">Tenable</span>
            <TenableIcon installed={item.tenable_installed_status} size={20}/>
            <span className="ml-2 text-sm text-gray-700">{item.tenable_installed_status ? 'Installed' : 'Not Installed'}</span>
          </div>
        </Section>

        {/* Host Details */}
        <Section icon={HardDrive} title="Host Details" color="text-violet-700">
          <InfoRow label="iDRAC" value={item.idrac_enabled ? `Enabled${item.idrac_ip?' ('+item.idrac_ip+')':''}` : 'Disabled'}/>
          <InfoRow label="OME Status" value={item.oem_status}/>
          <div className="py-2 border-b border-gray-50 flex items-start">
            ...
            {item.hosted_ip
              ? <div className="flex items-center gap-2">
                  <button onClick={() => navigate(`/physical-assets?ip=${encodeURIComponent(item.hosted_ip)}`)}
                    className="text-sm font-mono text-blue-700 hover:underline flex items-center gap-1">
                    <Server size={12}/>{item.hosted_ip}
                  </button>
        ...
        <Section icon={Key} title="Credentials" color="text-rose-700">
          <InfoRow label="Username" value={item.asset_username} mono/>
          <div className="flex items-center py-2">
        ...
        {item.additional_remarks && (
          <div className="card md:col-span-2">
            ...
            <p className="text-sm text-gray-700 whitespace-pre-wrap leading-relaxed">{item.additional_remarks}</p>
          </div>
        )}
        ...
        <div className="card md:col-span-2">
          <h3 className="font-semibold text-gray-800 mb-3 flex items-center gap-2">
            <Layers size={15} className="text-indigo-600"/> Audit Trail
          </h3>
          {auditLoading ? (
      ...
      <div className="mt-6 flex items-center justify-between text-xs text-gray-400 border-t border-gray-100 pt-4">
        <p>Submitted by <strong>{item.submitted_by||'—'}</strong></p>
        <p>
          Created: {item.created_at ? new Date(item.created_at).toLocaleString('en-GB',{day:'2-digit',month:'short',year:'numeric',hour:'2-digit',minute:'2-digit'}) : '—'}
        </p>
      </div>
    </div>
  );
